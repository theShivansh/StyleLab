...
| S13b | Live fix: jobs across instances | — | done | (this commit) | L T U I E B A | The deployed site showed an upload card failing with "That item isn't in your wardrobe" for a photograph that was being read correctly; the same upload succeeded when repeated, which was the clue. Job records lived in process memory, and during a rollout the upload and its poll reached different instances. Three fixes, each reproduced before it was made: `DatabaseJobStore` + migration 0003 (`JOB_BACKEND=database`), proven with two application instances over one database; a web poller that re-reads the garment on a job 404 and rides out a passing 5xx instead of ending the card on any error; and a `verify_schema` that tolerates what the database has and the models do not — without which the documented "migrate first" order would have stopped the live release surviving its next cold start. Also made `test_the_wardrobe_shows_items_that_are_still_analysing` deterministic: it asserted a transient state by racing a mock model and went red once under load. 690 api + 109 ai-eval + 82 web unit + 66 e2e (desktop + mobile). Live: upload, batch extraction, compose, swap, regenerate and save driven through the deployed site; every job poll 200. |
| S13c | Live fix: session identity, and the crew | — | done | (this commit) | L T U I E B A | Two defects the deployed site found, both reproduced against production before they were fixed. **Uploads:** the API's access log showed a `POST /session` before an upload, another before its job poll and another before the garment re-read — three anonymous users, and a card blaming the garment. `lib/session.ts` kept no copy of the token but localStorage, so a browser that refused site storage minted a user per request; and concurrent 401s each minted their own. The token is now held in memory for the page and renewed once per refused token. Reproduced on the live site with storage blocked. **Crew:** every deployed composition fell to the ranker. Reproduced locally against real Groq in the production shape — the Trend Scout on real Exa articles, which no live test had arranged: the scout's 600-token answer was cut off, Groq refused it (`json_validate_failed`), and one optional agent ended the crew; the crew also needed ~13k tokens against the account's 8,000 per minute and ran 48-83s against a 15s budget. Fixed with `AGENT_REASONING_EFFORT=low`, the provider's error code in the log, one retry with twice the ceiling for a truncated answer, CrewAI's blind re-runs off, optional roles failing onto the documented rung, a rebuild skipped and disclosed when it cannot finish in time, CrewAI's duplicated schema stripped from every prompt, compact handoffs. Also found on the way: the composition service overwrote the advisor's rung, so rung 3 had never reached a screen. Production-shaped crew, live: 8.1s, six agents, no refusal. **Live walkthrough after deploy** found four more, each reproduced and fixed: a regenerate timed out at the 15s budget (budget now 30s, base ceiling 600, rebuild cutoff a fifth — measured locally, both consecutive composes then served rung 1; live after the deploy, two back-to-back composes both came from the crew at rung 3, disclosed on the page, where before they fell to the ranker); a schema-invalid photo read was not retryable; re-uploading it returned the failed item from the checksum cache; and the poller drew a completed job over a failed item as "Read". 706 api + 126 ai-eval + 90 web unit + 68 e2e. |
| S13d | Live fix: a spent day read as a slow crew | — | done | (this commit) | L T | Compositions served the ranker again after S13c, and a 35s budget did not help. Reproduced locally with the deployed values; Groq's own 429 body said **tokens per day**: 199,257 of the free tier's 200,000 spent by a day of live testing, refilling at ~2.3 tokens a second — about one crew composition an hour for the whole app. The transport retried those refusals three times at ≤8s against requested waits of 3-11 minutes, so every compose burned its budget before falling back. A refusal whose wait is out of the retries' reach now fails at once, and the limit (`TPD`/`TPM`/`RPM`/`RPD`) and the requested wait are named in the transport, crew and composition log lines. The deployed env still had `AGENT_MAX_OUTPUT_TOKENS=800` and a no-op `CREWAI_TRACING_ENABLED=true`. The tier is the fix, and the operator's (B17). 709 api + 127 ai-eval; web unchanged. |
| S13e | Landing refresh | — | done | (this commit) | L T U I E | Hero rebuilt as two columns: the headline rises word by word, and a sample oxford-shirt photo is read through the real extraction stages with confidence marks, labelled an illustration. The two blank cards in "A guess is shown as a guess" now use the same photo, dimmed on the "before" card. First vendored components, from Vengeance UI at a pinned commit and rewritten to the repo's motion rules (`StaggerText`, `BorderBeam`); Skiper UI reviewed and not needed; Animmaster Lib still rejected. 90 web unit + landing e2e 18/18 (desktop + mobile), typecheck and lint clean. |
...
- [ ] B17 — Provider tier limits. Measured in S7 from a live response header plus the 429
      ...
      **output** tokens-per-minute ceiling of 1000 per model that the headers do not report.
      Three consequences, two already addressed:
        ...
        5. S13d found the limit that actually ended the crew in production: **200,000 text-model
           tokens per day**, refilled at ~2.3 tokens a second. A composition uses ~10,000, so a
           day of testing spends it and the whole app then gets about one crew composition an
           hour; every other one is the ranker. The transport now fails such a refusal at once
           and the log names it (`limit=TPD`). Only the paid Developer tier fixes it.
...
- [x] B10 — **REOPENED AND CLOSED AGAIN 2026-09-13.** It was closed in S5 on a hand
      ...
      **Nothing here excuses the original mistake**, and the lesson is not "trust the
      ...
      carries.
- [ ] B23 — **Row Level Security is off on every table in the Supabase project** (Supabase's
      own advisor, critical). The API is not exposed by it — it connects as the table owner
      through `DATABASE_URL`, and owners bypass RLS — but Supabase also serves every `public`
      table over its REST API to anyone holding the project's anon key, and `asset_blobs` now
      holds photographs. Enabling RLS with **no policies** closes that door without touching
      the application, for the same reason. Not applied from a session: it is a security
      setting on a production database, and the owner decides it. The SQL is one
      `ALTER TABLE … ENABLE ROW LEVEL SECURITY` per table, `jobs` included.
